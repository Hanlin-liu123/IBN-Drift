...
修复内容：
1. _get_interface_stats 通过节点 cmd() 在正确的网络命名空间内读取接口统计
2. 吞吐量同时使用 tx_bytes 和 rx_bytes 取最大值，修复单向为0的问题
3. 链路延迟从 ping 实测获取，不再使用随机数
"""
...
class MetricsCollector:
    ...
    def __init__(self,
        ...
        # 节点名称 -> Mininet节点对象的映射，用于在命名空间内执行命令
    ...
    def _get_interface_stats(self, intf_name: str, node_name: str) -> dict:
        ...
        修复说明：
        原来直接读宿主机 /sys/class/net/{intf}/statistics，
        Mininet 接口在独立的 netns 里，宿主机看不到，所以全是0。
        现在通过 node.cmd() 在节点的 netns 内读取。
        """
        stats = {}
    ...
    def _collect_link_metrics(self, snapshot: NetworkSnapshot):
        ...
        修复说明：
        1. 调用 _get_interface_stats 时传入节点名，在正确 netns 内读数据
        2. 吞吐量取 tx 和 rx 增量的较大值（修复单向统计为0）
        3. 链路延迟通过实际 ping 测量，不再使用随机数
        """
        if not self.network:
            return
        ...
        for link in self.network.links:
            ...
            # --- 修复3：链路延迟从 ping 实测（只对交换机间链路） ---
            # 主机-交换机链路延迟设为0，交换机间链路用 ping 测量
            if hasattr(src_node, 'dpid') and hasattr(dst_node, 'dpid'):
                # 两端都是交换机，通过各自连接的主机 ping 来估算
                # 简化方案：用链路配置的 delay 参数（从 tc 读取）
                try:
                    tc_out = src_node.cmd(
